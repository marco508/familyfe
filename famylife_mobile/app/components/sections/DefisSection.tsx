// components/sections/DefisSection.tsx
// ANNEXE V7 — Corps de l'ancien écran `(app)/defis.tsx`, extrait pour être rendu
// comme segment du hub Équité. Ne rend ni fond ni en-tête : l'action "créer"
// passe du bouton + de l'en-tête à un FAB, l'en-tête appartenant au hub.
// Défis de maison (ANNEXE V3) : rejoindre, marquer terminé (gagne les points),
// création + clôture pour la gestion (chef/co-chef).
...
import { Plus, Lock, Trash2, Check } from 'lucide-react-native';
...
import { Avatar, Badge, BottomSheet, CandyButton, CandyCard, CandyInput, EmptyState, Fab, Stepper, VisitorBanner } from '../ui';
...
export default function DefisSection({ bottomInset = spacing['4xl'] }: Props) {
      ...
      >
        ...
        {loading ? (
          ...
          <EmptyState
            emoji="🏆"
            title={t('defis.aucun')}
            message={!isVisiteur ? t('defis.creerBouton') : undefined}
          />
        ) : (
          defis.map((d) => (
            <CandyCard key={d.id} style={styles.card}>
              ...
              <View style={styles.metaRow}>
                <Badge
                  label={d.statut === 'ouvert' ? '🟢 ' + t('defis.ouvert') : '🔒 ' + t('defis.ferme')}
                  variant={d.statut === 'ouvert' ? 'green' : 'neutral'}
                />
                {d.date_fin ? (
